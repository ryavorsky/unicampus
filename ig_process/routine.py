# -*- coding: utf-8 -*-
"""
Created on Sat Aug 22 11:40:59 2015
...
db = check_db_likes(db, ig_api)
db = check_db_stopwords(db, stop_list)
 
media_id_dict = {}
for tag in tags_list:
    try:
        tagtime = datetime.datetime.fromtimestamp(tagtimes[tag])
    except:
        tagtime = datetime.datetime.fromtimestamp(0)
        
    lst = load_media_list_by_tag(tag, ig_api, tagtime)
    media_id_dict = process_media_list(lst, media_id_dict, db, stop_list)
         
db.update(media_id_dict) 

db_week, db_month = make_wm_db(db)

weeklist = top20_json_list(db_week)
monthlist = top20_json_list(db_month)
alllist = top20_json_list(db)

tagtimes = {}
for tag in tags_list:
    almost_now = datetime.datetime.now() - datetime.timedelta(minutes = 1)
    tagtimes.update({tag: int(almost_now.timestamp())})

cinfo = connect_to_server()
upload_to_server(cinfo, db, weeklist, monthlist, alllist, tagtimes)
break_connection(cinfo)
